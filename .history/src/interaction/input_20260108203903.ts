...
export async function deviceType(
  ...
  if (platform === "android") {
    ...
    if (isAscii) {
      ...
      // Escape content is tricky in shell, but basic " should be safe enough if we wrap in "
      // Actually, we need to escape ' and " and ( ) etc if we are shell execing.
      // But adb input text expects space as %s.
      await execAsync(`adb -s ${deviceId} shell input text "${escapedText}"`);
    } else {
      // Fallback to Maestro for Unicode/Complex text (e.g. Cyrillic)
      // Maestro handles text injection much more robustly
      const flowYaml = `
---
- inputText: "${text}"
`;
