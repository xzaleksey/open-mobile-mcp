...
export async function deviceType(
  ...
  if (platform === "android") {
    ...
    if (isAscii) {
      ...
      // Strategy: Use Maestro (User Recommendation)
      // "Maestro everywhere" is the most reliable method for Unicode on Android
      // as it uses Accessibility APIs, bypassing the flaky 'input text' command.

      console.error(
        `[DeviceType] Unicode Strategy: Maestro (Escaped) for '${text}'`
      );

      // Escape characters to \uXXXX to ensure the YAML file is pure ASCII.
      // This prevents Windows shell/filesystem encoding corruption.
